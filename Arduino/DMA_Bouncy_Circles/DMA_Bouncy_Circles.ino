...
#define DWIDTH  128  // modified
#define DHEIGHT 160  // modified
...
void setup() {
  ...
  for (uint16_t i = 0; i < CNUMBER; i++) {
    ...
    circle->col[i] = rainbow(255/CNUMBER * i);         // 4 -> 255/CNUMBER
    circle->dx[i] = random(1, 3);                      // 5 -> 3
    if (random(2)) circle->dx[i] = -circle->dx[i];
    circle->dy[i] = random(1, 3);                      // 5 -> 3
