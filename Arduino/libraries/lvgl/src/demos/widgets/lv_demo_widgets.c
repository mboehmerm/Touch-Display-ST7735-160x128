...
void lv_demo_widgets(void)
{
    ...
    if(disp_size == DISP_LARGE) {
        ...
        font_large     = &lv_font_montserrat_12;                                                                   //// geändert
#else
        LV_LOG_WARN("LV_FONT_MONTSERRAT_12 is not enabled for the widgets demo. Using LV_FONT_DEFAULT instead.");
#endif
#if LV_FONT_MONTSERRAT_10
        font_normal    = &lv_font_montserrat_10;                                                                   //// geändert
#else
        LV_LOG_WARN("LV_FONT_MONTSERRAT_10 is not enabled for the widgets demo. Using LV_FONT_DEFAULT instead.");
#endif
    }
...
void shop_create(lv_obj_t * parent)
{
    ...
    if(disp_size == DISP_LARGE) {
        ...
        lv_obj_set_width(chart3, LV_PCT(95));                  //// modify ?
    ...
    if(disp_size == DISP_SMALL) {                              //// modify ?
...
static void color_changer_create(lv_obj_t * parent)                                                //// Color Changer modified
{
    ...
    lv_obj_set_style_pad_right(color_cont, disp_size == DISP_SMALL ? LV_DPX(43) : LV_DPX(55), 0);  //// modified 47 -> 43
    lv_obj_set_style_bg_opa(color_cont, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(color_cont, LV_RADIUS_CIRCLE, 0);

    if(disp_size == DISP_SMALL) lv_obj_set_size(color_cont, LV_DPX(42), LV_DPX(42));               //// modified 52 -> 42
    ...
    for(i = 0; palette[i] != _LV_PALETTE_LAST; i++) {
        ...
        lv_obj_set_size(c, 16, 16);                                                                //// modified -4
    ...
    if(disp_size == DISP_SMALL) {
        lv_obj_set_size(btn, LV_DPX(38), LV_DPX(38));                              //// modified -4
        lv_obj_align(btn, LV_ALIGN_BOTTOM_RIGHT, -LV_DPX(12), -LV_DPX(12));        //// modified -3
    }
    else {
        lv_obj_set_size(btn, LV_DPX(50), LV_DPX(50));                              //// modified -10
...
{
    lv_obj_t * obj = var;
    lv_coord_t max_w = lv_obj_get_width(lv_obj_get_parent(obj)) - LV_DPX(16);      //// modified -4 -4
    lv_coord_t w;

    if(disp_size == DISP_SMALL) {
        w = lv_map(v, 0, 256, LV_DPX(44), max_w);                                  //// modified -4 -8
    ...
    if(code == LV_EVENT_FOCUSED) {
        if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_POINTER) {
            if(calendar == NULL) {
                ...
                if(disp_size == DISP_SMALL) lv_obj_set_size(calendar, 160, 180);                  //// modified -20 -20
